...
<div class="rdefuncion">
  ...
  <div class="cflex">
    ...
    <span style="order: 2; flex-grow: 1; margin-right:10px;">
      <div class="form-group">
        <?= Html::label('Documento del Fallecido', 'doc_fall') ?>
        <?= Html::radioList('Defuncion[tipo_doc]','Documento Único de Identidad',['Documento Único de Identidad'=>'Documento Único de Identidad','Partida de Nacimiento'=>'Partida de Nacimiento'],['id'=>'doc_fall']) ?>
      </div>
    </span>
  </div>
  <div class="cflex">
    <span style="order: 1; flex-grow: 1; margin-right:10px;">
      <?= $form->field($model, 'alc_partida')->dropDownList(ArrayHelper::map(Municipio::find()->all(), 'nombre', 'nombre'),['disabled'=>'disabled']) ?>
      <div class="form-group">
        <?= Html::textInput('falcaldia','',array('id'=>'nrwr4','class'=>'form-control')); ?>
        <span id="matches4" style="display:none"></span>
      </div>
    </span>
    <span style="order: 2; flex-grow: 1; margin-right:10px;">
      <?= $form->field($model, 'datos_partida')->textArea(['readonly'=>true,'rows'=>3,'style'=>'resize:none;']) ?>
    </span>
  </div>
  <?= Html::hiddenInput('Defuncion[familiares]','',['id'=>'ifam']); ?>
  <div class="cflex">
    <?php
      $model->con_asistencia = 1;
    ?>
    <span style="order: 1; flex-grow: 1; margin-right:10px;">
      <?= $form->field($model, 'con_asistencia')->radioList([1=>'Si',0=>'No']) ?>
    </span>
    <span style="order: 2; flex-grow: 1; margin-right:10px;">
    ...
    <span style="order: 3; flex-grow: 1; margin-right:10px;">
      <?= $form->field($model, 'determino_causa')->textArea(['rows'=>3,'style'=>'resize:none;']) ?>
    </span>
  </div>
  <div class="cflex">
  ...
  <div class="cflex">
    ...
    <span style="order: 3; flex-grow: 1; margin-right:10px;">
      <?= $form->field($partida, 'lugar_suceso')->textArea(['rows'=>3,'style'=>'resize:none;','placeholder'=>'Especifique el lugar'])->label('Lugar de Defuncion') ?>
    </span>
  </div>
  <div class="cflex">
    <span style="order: 1; flex-grow: 1; margin-right:10px;">
